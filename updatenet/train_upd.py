...
tem_path ='../updatenet/update_set_templates_step1_iou_0.1_std'
...
dataram['train'] = np.arange(len(dataram['gt']), dtype=np.int_)#生成0到len(50)之间的随机数字
...
def adjust_learning_rate(optimizer, epoch, lr0):
    lr = np.logspace(-lr0[0], -lr0[1], num=args.epochs) #构造一个从10的负lr0[0]次方到10的lr0[1]次方的等比数列，数列长度是50
...
for ii in np.arange(0,1):
    ...
    model.load_state_dict(torch.load('./models/checkpoint7.pth.tar')['state_dict'], strict=False)
        ...
        for t in range(0, len(subset), args.batch_size):  # start：0； stop：len(subset)=45578；  step:batch_size=64
            ...
            for rr in range(len(init_index)):
                if init_index[rr] != 0:  # 不是初始帧
                    init_index[rr] = np.random.choice(init_index[rr],
            ...
            # pdb.set_trace()
            temp = np.concatenate((init, pre, cur), axis=1)  # (64, 2304, 7, 7)
            ...
            target = Variable(target).cuda()  # [64,512,6,6]

            # compute output
            output = model(input_up, init_inp)  # [64, 2304, 7, 7]   [64, 768, 7, 7]
