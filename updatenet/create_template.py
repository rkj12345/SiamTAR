...
from pysot.utils.bbox import get_axis_aligned_bbox, cxy_wh_2_rect1, overlap_ratio
from tracker_upd import SiamCAR_init_upd,SiamCAR_track_upd

parser = argparse.ArgumentParser(description='SiamCAR_updatenet_create_template')
...
if step>1: #第一阶段是线性更新,第二阶段开始使用UpdateNet网络进行更新
    ...
    #updatenet = nn.DataParallel(updatenet).cuda()

    update_model=torch.load('../updatenet/checkpoint/checkpoint6.pth.tar')['state_dict']
...
temp_path ='../updatenet/'+setfile+'_templates_step'+str(step)+'_iou_0.1'+'_std'  #step=1,2,3
...
list_file = [line.strip() for line in lists] #读取 update_set_test.txt
...
for video in category:
    ...
    while frame < num_frames:
        ...
        if w * h != 0:  # xywh2cxcywh不存在等于0的情况，polygon2cxcywh存在越界的情况
            ...
            state = SiamCAR_init_upd(im, target_pos, target_sz, model)
            ...
            while frame < (num_frames - 1):
                ...
                im = cv2.imread(image_file)  # HxWxC
                state = SiamCAR_track_upd(state, im, updatenet)
                ...
                if frame == (num_frames - 1):  # last frame
                    ...
                    gt.append(1)
                res = cxy_wh_2_rect1(state['target_pos'], state['target_sz'])  # zero-based
                # 计算当前网络检测出来的框和GT框的iou，如果不相交则丢失目标
                if reset:  # reset 默认是1
                    ...
                    if iou <= 0.1:  ##这个条件是不是太宽了 iou<0.5才可以把,you can choose iou<0.2, iou<0.3,  iou<0.4.
                        break
        else:#
...
np.save(temp_path+'/template',template_acc) #累积特征图 [1,512,6,6]  第i帧预测结果
np.save(temp_path+'/templatei',template_cur)#当前的检测模板 [1,512,6,6] 第i-1帧最佳模板
np.save(temp_path+'/template0',template_gt) #gt模板 [1,512,6,6]  第一帧GT
np.save(temp_path+'/init0',init0)#第一帧  [0,1,2...]
np.save(temp_path+'/init',init)#累积模板编号   形式为：[0,1,2.....]
np.save(temp_path+'/pre',pre)#每一帧对应的检测模板 ,形式为：[0,1,1,...]
np.save(temp_path+'/gt',gt)#每一帧对应的gt,一般为1，形式为：[1,1,1,1,......]
