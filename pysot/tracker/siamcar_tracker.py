...
from pysot.utils.misc import bbox_clip, bbox_clip_rf
...
class SiamCARTracker(SiameseTracker):
    ...
    def _convert_cls(self, cls):
        ...
        return cls
    def get_ar(self, img, init_box, ar_path):
        """ set up Alpha-Refine """
        selector_path = 0
        sr = 2.0;
    ...
    def init(self, img, bbox):
        ...
        if self.step ==1:
            ...
            ar_path = 'D:\\codeProjects\\SiamCAR_RGBT_alpha_refine1\\tools\\snapshot\\SEcmnet_ep0040-c.pth.tar'
            self.RF_module = self.get_ar(img, bbox, ar_path)
        else:
    ...
    def track(self, img, hp ):
        ...
        bbox = [cx - width / 2,
                ...
                width,
                height]
        # pred_bbox = self.RF_module.refine(img, np.array(bbox))
        # x1, y1, w, h = pred_bbox.tolist()
        # H, W, _ = img.shape
        # x1, y1, x2, y2 = bbox_clip_rf(x1, y1, x1 + w, y1 + h, (H, W))
        # w = x2 - x1
        # h = y2 - y1
        # # # print(bbox)
        self.center_pos = np.array([x1 + w / 2, y1 + h / 2])
        ...
        if self.step>1:
            ...
            if self.step==2:#模板更新方式1-Linear
                ...
                temp = torch.cat((Variable(state['z_0']).cuda(),Variable(state['z_f']).cuda(),z_f),1) #3个[1,768,7,7]合并为[1, 2304, 7, 7]
