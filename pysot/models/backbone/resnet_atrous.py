...
from ..attention.se_module import se_block
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
        ...
        self.planes = planes
        # if self.planes == 128:
        #     self.ema1 = Enhanced_multi_atten(512)
    ...
    def __init__(self, block, layers, used_layers):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
#         self.ema1 = Enhanced_multi_atten(512)
#         self.ema2 = Enhanced_multi_atten(1024)
#         self.ema3 = Enhanced_multi_atten(2048)
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x_ = self.relu(x)
        x = self.maxpool(x_) #[1, 64, 31, 31]

        p1 = self.layer1(x)# [1, 256, 31, 31]

        p2 = self.layer2(p1)#[1, 512, 15, 15]

        #p2 = p2 + self.ema1(p2)
        #p2 = self.relu(p2)

        p3 = self.layer3(p2)#[1, 1024, 15, 15]
        #p3 = p3 + self.ema2(p3)
        #p3 = self.relu(p3)

        p4 = self.layer4(p3)#[1, 2048, 15, 15]
