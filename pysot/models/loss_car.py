"""
This file contains specific functions for computing losses of SiamCAR_3080_checkpot_e19
file
"""
...
class IOULoss(nn.Module):
    def forward(self, pred, target, weight=None, eps=1e-9):
        pred_left = pred[:, 0]
        pred_top = pred[:, 1]
        pred_right = pred[:, 2]
        pred_bottom = pred[:, 3]
        ...
        # S = ((target_left - pred_left) ** 2) + ((target_top - pred_top) ** 2) + ((target_right -pred_right ) ** 2) + ((target_bottom - pred_bottom) ** 2)
        # inter_x1, inter_y1 = torch.min(target_left, pred_left), torch.min(target_right, pred_right)
        # inter_x2, inter_y2 = torch.min(target_top, pred_top), torch.min(target_bottom, pred_bottom)
        # I = (inter_x1 + inter_x2 - 1) ** 2 + (inter_y1 + inter_y2 - 1) ** 2
        #
        # cw = torch.max(target_left, pred_left) + torch.max(target_right, pred_right) - 1  # convex (smallest enclosing box) width
        # ch = torch.max(target_top, pred_top) + torch.max(target_bottom, pred_bottom) - 1  # convex height
        # C = cw ** 2 + ch ** 2 + eps
        #
...
class SiamCARLossComputation(object):
    """
    This class computes the SiamCAR_3080_checkpot_e19 losses.
    """
