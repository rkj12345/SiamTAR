...
import sys
print(sys.path)
sys.path.append('D:\codeProjects\SiamCAR-master-updateneting')
import torch
...
def build_opt_lr(model, current_epoch=0):
        ...
        for layer in cfg.BACKBONE.TRAIN_LAYERS:
            ...
            for m in getattr(model.backbone, layer).modules():
                if isinstance(m, nn.BatchNorm2d):
                    m.train()
    else:     #前10个epoch冻结backbone进行训练
        for param in model.backbone.parameters():
            param.requires_grad = False
        for m in model.backbone.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()#冻住bn层，获得可以稳定输出的结果

    trainable_params = []
    trainable_params += [{'params': filter(lambda x: x.requires_grad,    #在迁移学习时需要冻结部分网络层的参数, 可以选择只给requires_grad=TRUE的参数设置优化器
    ...
    lr_scheduler.step(cfg.TRAIN.START_EPOCH)  #根据epoch更新学习率
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            if get_rank() == 0:
                torch.save(
                        ...
                        cfg.TRAIN.SNAPSHOT_DIR+'/checkpoint_test_e%d.pth' % (epoch))
        ...
        if rank == 0:
            tb_writer.add_scalar('time/data', data_time, tb_idx)  #将我们所需要的数据保存在文件里面供可视化使用

        outputs = model(data) #data:一个字典{'template', 'search', 'label_cls', 'bbox' }，bbox:[32, 4]
        #label_cls:[32,25,25]   search:[32, 3, 255, 255]     template:[32, 3, 127, 127]
...
def main():
    ...
    if rank == 0:
        ...
        init_log('global', logging.INFO)  #初始化日志信息
    ...
    dist_model = nn.DataParallel(model).cuda()  #多张gpu训练设置
    ...
    dist_model = nn.DataParallel(model) # 当不限制GPUs的个数时，默认使用全部的GPU
